$ExclusiveProductIDs = @('O365ProPlusRetail','O365BusinessRetail','ProPlus2019Volume','Standard2019Volume','ProfessionalRetail','O365HomePremRetail')
$OfficePrograms = @(@{Name="Microsoft Outlook";FileName="Outlook.exe";Shortname="outlook"},@{Name="Microsoft Excel";FileName="Excel.exe";Shortname="excel"},@{Name="Microsoft Powerpoint";FileName="Powerpnt.exe";Shortname="PowerPoint"},@{Name="Skype for Business";FileName="lync.exe";Shortname="Skype for Business"},@{Name="Microsoft Word";FileName="WinWord.exe";Shortname="word"}) | ConvertTo-Json | ConvertFrom-Json
if($ForceAppClosure -eq $true)
{
    Write-Host "Force App Closure Selected, closing Office Programs"
    foreach($OfficeProgram in $OfficePrograms)
    {
        Write-Host "Closing $($OfficeProgram.Name)"
        taskkill /im $OfficeProgram.FileName /F 2>&1 | Out-Null
    }
}
#region Functions
Function Get-Office365ChannelName
{
    ...
    $DesiredChannel = switch($FriendlyChannelName)
    {
        ...
        "Semi-Annuel Enterprise Channel" {"SemiAnnualPreview"}
    }
...
else
{
    if($UpdateChannel)
    {
        ...
        Write-Debug "Configured Update Channel: $ConfiguredChannel"
        Write-Debug "Desired Update Channel: $DesiredChannel"
...
if($SoftwareName -like "Microsoft 365 Apps for*" -or $SoftwareName -like "Office 2019*" -or $SoftwareName -like "Office 2016*")
{
    ...
    if($null -ne $OfficeInstallations -and $OfficeInstallationCount -gt 0)
    {
...
{
    $ProofingToolsXML = '<Product ID="ProofingTools"><Language ID="MatchOS" /></Product>'
}
$DownloadConfig = @"
<Configuration>
...
[xml]$InstallConfigxml = $InstallConfig 
Write-Host $InstallConfigxml
New-Item -ItemType File -Path "$TargetPath\InstallConfig$ProductID.xml" -Value $InstallConfigxml.InnerXml -Force -ErrorAction SilentlyContinue | Out-Null
...
# run batch to Install
Write-Host $InstallConfig
...
Function New-DesktopShortcut
{
    ...
    if(Test-Path $TargetPath)
    {
        ...
        {
            # Write-Host "Overwriting existing shortcut for $Name"
        }
    ...
    {
        Write-Host "Unable to find $TargetPath"
    }
}

foreach($Program in $OfficePrograms)
{
    New-DesktopShortcut -Name $Program.name -TargetPath (Join-Path -Path "$InstallPath\root\Office16" -ChildPath $Program.FileName)
}
...
{
    $LinesToShow = 150
    "No interesting logs found. Dumping last $LinesToShow lines"
    $Logs | select -Last $LinesToShow
}
